...
export class MeetingLoopService {
  ...
  // R14.3 / ADR-047: work-item edit, responsibility correction, and addition
  // moved to the kernel commands `meeting.editWorkItem`,
  // `meeting.correctWorkItemResponsibility`, and `meeting.addWorkItem`. A
  // remote operator reaches the Hub, where this device-local state does not
  // exist, and keeping a second writer here would give one meeting two
  // concurrency models. The transformations live in `meeting-work-items.ts`,
  // which both this package's tests and the kernel handler use.
