...
export const KNOWN_HEADING_TYPOGRAPHY_DEBT = [
  ...
  //   `className` will ever return.
  {
    ...
    note: "styles.css:8092, inside the `.document-canvas` block that opens at :8063. The note canvas sets its own font-size as a clamp and then leaves prose headings on the UA scale, so an <h1> in a note draws at 2em of that clamp. Three subjects, one rule.",
  },
  {
    sheet: "styles.css",
    ...
    missing: ["font-size", "font-weight"],
    owner: "lot 5 — file scope",
    note: "styles.css:8092 — same rule, UA 1.5em.",
  },
  {
    sheet: "styles.css",
    ...
    missing: ["font-size", "font-weight"],
    owner: "lot 5 — file scope",
    note: "styles.css:8092 — same rule, UA 1.17em.",
  },
  ...
  },
  {
    ...
    missing: ["font-size", "font-weight"],
    owner: "lot 6 — file scope",
    note: "styles.css:10270. This rule is a REFINEMENT of `.concept-help-dialog h2` (:10100) and would not be judged at all if that rule were complete — but it declares no weight either, so nothing covers this one and both are registered. Giving :10100 a weight retires BOTH entries in one edit, and the scan will demand exactly that: this subject turns into a `refinement` the moment the general rule declares both.",
  },
  {
    sheet: "styles.css",
  ...
  },
  {
    ...
    missing: ["font-weight"],
    owner: "lot 6 — file scope",
    note: "styles.css:10344 — same rule, second ancestor.",
  },
  {
    ...
    missing: ["font-size", "font-weight"],
    owner: "lot 6 — file scope",
    note: "activity-section.module.css:467. A CLASS subject (`ActivitySection.tsx:66` puts it on an <h3>) whose only rule sets `min-width: 0`. The layout gate already knows this element by name — `h3._stateTitle` was a registry entry in `descendant-overflow.mjs` — so it is measured for WIDTH and unmeasured for TYPE.",
  },
  ...
  },
  {
    ...
    missing: ["font-size", "font-weight"],
    owner: "outside Phase 3",
    note: "styles.css:4428. The loading and failure state of EVERY lazy surface, so it is seen on the way into most screens. `.center-state h1` (:4407) is the same shape done right (--text-xl / --weight-semibold, which resolves to 600 in tokens.css:72) and the two classes never co-occur.",
  },
  {
    sheet: "styles.css",
    ...
    missing: ["font-size", "font-weight"],
    owner: "outside Phase 3",
    note: "styles.css:7547. Co-occurs with `inspector-empty`, which has no heading rule at all, so nothing covers it.",
  },
  {
    sheet: "styles.css",
    ...
    missing: ["font-weight"],
    owner: "outside Phase 3",
    note: "styles.css:7773 — outside the Settings region lot 6 owns.",
  },
  {
